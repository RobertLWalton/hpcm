...
# Date:		Thu Jul 19 00:27:00 EDT 2001
#
...
set problems_per_line 8

# Output labeling lines.
#
proc label_problems {} {

    global sorted_problems spaces32 separators \
           problems_per_line

    set ppl $problems_per_line
    ...
    foreach problem $sorted_problems {
		...
		if { $count == 0 } {
		    ...
		    set line \
		         [string range $spaces32 0 15]
		}
	    }
	    incr count
	    set line "$line[format { %7.7s} $problem]"
    }
    puts [translit " " \
	     [string index $separators \
		     [expr { ( $count + $ppl - 1 )
		             / $ppl - 1 } ]] \
	     [string range $line$spaces32$spaces32 \
	             0 79]]
    if { $count <= $ppl } {
        puts [translit \
	        " " [string index $separators 1] \
	        [string range \
		        "[format {%16.16s} \
			         "correct/tot-time"]\
			$spaces32$spaces32" \
...
foreach item [lsort -decreasing $score_list] {
	...
	if { $count % $ppl == 0 } {
	    ...
	    if { $count == 0 } {
	        set line [format {%16.16s} $submitter]
	    } elseif { $count == $ppl } {
	        set line [format {%16.16s} \
		                 $total_score]
	    } else {
	        set line [string range $spaces32 0 15]
	    }
	}
	incr count
