...
# Date:		Sun Nov  5 07:51:14 EST 2000
#
...
scoreboard \[parameters_file\]
    ...
    time_score is the sum of all the correct problem
    times, plus a penalty times the number of incorrect
    submissions that are for a problem for which there
    is a later correct submission.  All times are actu-
    ally measured in seconds, even though the problem
    times are often reported on the scoreboard itself
    only to the nearest minute or hour.  The penalty is
    the value of `scoreboard_penalty', and is in sec-
    onds.
...
proc label_problems {} {
    ...
    global sorted_problems spaces32
...
foreach item [lsort -decreasing $score_list] {
    ...
    # If we have a change in the number of problems
    # a team has correct, print a new header for the
    # new group of teams with a given number of
    # problems correct.
    #
    if { $problems_correct \
