/* Programming Contest Sendmail and Finish Program 
 *
 * File:	hpcm_sendmail.c
 * Authors:	Bob Walton (walton@deas.harvard.edu)
 * Date:	Sat Oct 19 05:03:26 EDT 2013
 *
...
/* This program can be called by the name hpcm_sendmail
 * to send mail or the name hpcm_finish to mark a team
 * finished with a contest.
 */
...
char finish_message [] =
"\n"
"    If you finish a contest `*DONE*' will appear on\n"
"    the scoreboard next to your total score and any\n"
"    additional submissions that you make will be\n"
"    ignored.  You may then get extra help from the\n"
"    host site contest managers on problems.\n"
"\n"
"    The ONLY reason for doing this is if you are\n"
"    stuck on all remaining problems and want to\n"
"    work on some of these for fun or education.\n"
"    If you simply want to leave the contest you\n"
"    should NOT use this program.  Also, it is\n"
"    strongly recommended that you NOT finish until\n"
"    you have solved at least one problem.\n"
"\n"
"    Lastly note that the host site managers may not\n"
"    be able to give you help on the hardest\n"
"    problems.\n" ;
...
int main ( int argc, char ** argv )
{
    ...
    int finish =
        ( strcmp ( argv[0] + strlen ( argv[0] ) - 11,
	           "hpcm_finish" ) == 0 );
...
#   define MAXLEN 400
    ...
    */
    ...
    /* Signature: field value:
       ...
       if this is NOT a finishing message, or
       ...
       if this IS a finishing message.
	...
	* p = '\0';
    ...
    /* If we are hpcm_finish check if user really wants
     * to continue
     */
    if ( finish )
    {
        printf ( "%s", finish_message );
	while ( 1 )
	{
	    ...
	    if ( strcmp ( "no", buffer ) == 0 )
	        exit ( 0 );
	    else if ( strcmp ( "yes", buffer ) == 0 )
	        break;
	    else
	        printf ( "`%s' is not `yes' or `no'\n",
		         buffer );
	}

	printf ( "\n"
	         "You need to call a host site manager"
		 " to enter the following"
		 " password.\n" );

	while ( 1 )
	{
	    printf ( "\nEnter the special finishing"
	...
	if ( ! finish )
	    fprintf ( files[0], "Cc:%s\n", cc );

	/* Read and copy any header, stopping at empty
	   line after header.  On `finish' replace
	   input with finishing header.
	*/
	if ( finish )
	    strcpy ( line,
	             "Subject: ***FINISHED***\n" );
